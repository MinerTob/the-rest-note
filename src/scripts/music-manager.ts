...
export class MusicManager extends EventTarget {
   ...
   *
   * 为什么需要（历史提交 404cf58 定位过）：自动恢复有好几个入口（`init()` 那一枪、
   * `unlockAll()` → `retryIfIdle()`、`canplay` / `pageshow` / `visibilitychange`），
   * 它们以前各自直接 `attemptStart()`，同一瞬间可以并发好几次 `el.play()`。
   * 并发的结果不是"多试一次"，而是**互相打架**：后一次把前一次打断（AbortError），
   * 先失败的那次还会把状态写回 `ready`，于是 UI 显示"暂停"而元素其实在响 ——
   * 用户点一下播放键反而把它 pause 掉（"响一下马上停"），并且那次 pause 把
   * `space.paused` 写成 true，之后每次刷新都不再自动恢复。
   *
   * 用计数而不是布尔：旧事务被显式 `play()` 顶替时仍要能安全收尾，
